...
export default function MovieSearch() {
  ...
  //useInput 훅을 사용해ㅐ 사용자 입력 값을 상태로 관리 
  const searchInput = useInput('')
  const searchTerm = searchInput.props.value

  // Debounce 검색
  // 사용자가 입력을 멈춘 후, 특정 시간이 지나도 입력을 하지 않을 시에 데이터 요청
...
const {isLoading, error, data, refetch} = useFetch<ResponseMovieData>({
  url: getTmdbQuery(debounceSearch), // 검색어가 있을 시 검색 영화 목록, 없을 시 인기 영화 엔드포인트 반환
          ...
          >
            <div className={S.posterWrapper}>
              <div className={S.badgeGroup}>
                ...
                <span className={`${S.badge} ${S.lang}`}>{movie.original_language}</span>
              </div>
            ...
            <div className={S.info}>
              ...
              <p className={S.overview}>
                {movie.overview ||'영화에 대한 상세 설명이 이곳에 표시됩니다. 줄거리가 길어지면말줄임표 처리가 되는지 확인해보세요.'}
              </p>
