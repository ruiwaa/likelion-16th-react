...
export default function EffectDependencies() {
  ...
  // 이펙트 설정 (종속성 또는 의존성)
  // 상황 1 의존성 배열이 없을 때
  // 컴포넌트가 렌더링 할 때 마다 이펙트 함수의 로직이 실행됨
  // useEffect(() => {
  //   document.title = `⌨️ ${text.length === 0 ? '입력 대기 중 ...' : text}`
  //   console.log('외부의 시스템인 브라우저와 리액트의 상태를 동기화했습니다. 👏');
    
  // })

  // 상황 2 의존성 배열이 비어있는 경우
  // 최초 렌더링 할 때 단 한번만 이펙트 함수의 로직이 실행됨
  useEffect(() => {
    const currentYear = new Date().getFullYear()
    setCount(currentYear)
    console.log(`올해는 ${currentYear} 입니다.`);
  }, []) // 종속성을 비워줌

  // 상황 3 의존성 배열에 특정 값이 포함된 경우
  // 종속성 배열에 반응성 데이터를 추가하면 오직 그 데이터가 변경될 때만 함수 로작이 실행됨.
  useEffect(() => {
