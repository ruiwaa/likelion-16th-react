...
export default function MemoizationValue() {
  ...
  // useState 안에 기억된 함수가 아니여서, 
  // 일반 함수는 렌더링 될때마다 새로 생성된다.
  // 여기서는 객체이기때문에 useCallback을 사용할 수 없음
  // useMemo를 사용!

  // 한 줄로 사용하려면 객체를 () 안에 넣어줘야함
  ...
  // ✏️ useRef(돔 참조), useMemo(객체 참조), useCallback(함수 참조)

  // const 함수값 = () => setCount((prev) => prev + 1)
// useMemo "자바스크립트의 모든 값을 기억한다"
// const onIncreamentMemo = useMemo(() => () => setCount((prev) => prev + 1), [])

// const callbackRef = useRef(() => setCount((prev) => prev + 1)) // { current: 함수참조 }

// useCallback "함수 값만 기억한다"
// const onIncreamentCallback = useCallback(() => setCount((prev) => prev + 1), [])
...
// 비용이 많이 드는 계산
  // - getExpensiveValue
  // - computedTime
  const calcurateTime = useMemo(() => computedTime(() => getExpensiveValue(count)), [count])
