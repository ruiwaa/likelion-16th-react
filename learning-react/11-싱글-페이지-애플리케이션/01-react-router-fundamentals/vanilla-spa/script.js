...
function render() {
  ...
  const planet = params.get("planet") ?? "미정";
...
function navigateTo(path, state = null) {
  ...
  // 글로벌 객체는 popState 이벤트 감지 이벤트 핸들러 연결 [수신]
  globalThis.dispatchEvent(navigateEvent);
  // 브라우저가 받은 navigateEvent가 발동할 때마다 화면을 다시 그림
}
