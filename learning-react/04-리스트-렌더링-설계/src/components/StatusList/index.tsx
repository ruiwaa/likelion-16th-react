...
export default function StatusList() {
  ...
  const handleFetchData = async () => {
    ...
    if(isLoading) return // 로딩 중이니깐 작동하면 안됨
    // 로딩중... 상태로 전환
    setIsLoading(true)
  ...
  return (
    <section className={S.container}>
      ...
      <div role="group" className={S.buttonGroup}>
        <button type="button" onClick={handleFetchData} aria-disabled = {isLoading}>데이터 가져오기</button>
        <button type="button" onClick={handleReset} aria-disabled = {isLoading}>데이터 초기화</button>
      </div>
    </section>
  )
}
