...
const UserSchema = z.object({
  name: z.string('사용자 이름은 문자 값이어야 합니다. ').min(2),
  age: z
    .number()
    .min(19, '사용자  나이는 최소 19세 이상이어야 합니다.')
    .max(59, '사용자 나이는 최소 59세  이하여야 합니다.'),
  job: z.string('사용자 직업은 문자값이어야 합니다.'),
  email: z.email('유효하지 않은 이메일 주소입니다.'),
  phoneNumber: z
    .string('사용자 전화번호는 문자 값이어야 합니다.')
    .regex(/^010-\d{4}-\d{4}/, '유효하지 않은 전화번호입니다.'),
  github: z
    .string('Github 주소는 문자값이어야 합니다.')
